...
local function install()
	...
	if (TukuiCF["Chat"].enable) and (not IsAddOnLoaded("Prat") or not IsAddOnLoaded("Chatter")) then					
		...
		for i = 1, NUM_CHAT_WINDOWS do
			...
			frame:SetSize(TukuiDB.Scale(TukuiCF["Panels"].infowidth - 8), TukuiDB.Scale(111))
			
			-- this is the default width and height of tukui chats.
			SetChatWindowSavedDimensions(chatFrameId, TukuiDB.Scale(TukuiCF["Panels"].infowidth - 8), TukuiDB.Scale(111))
			
			-- move general bottom left or Loot (if found) on right.
			if i == 1 then
				frame:ClearAllPoints()
				frame:SetPoint("BOTTOMLEFT", TukuiDataLeft, "TOPLEFT", TukuiDB.Scale(3), TukuiDB.Scale(8))
			elseif i == 4 and chatName == "Trade / Loot" then
				frame:ClearAllPoints()
				frame:SetPoint("BOTTOMRIGHT", TukuiDataRight, "TOPRIGHT", TukuiDB.Scale(-5), TukuiDB.Scale(8))
			end
...
TukuiOnLogon:SetScript("OnEvent", function(self, event)
	...
	if TukuiCF["Actionbars"].enable then
		SetActionBarToggles(1, 1, 1, 1, 0)
		SetCVar("alwaysShowActionBars", 0)	
		if TukuiCF["Actionbars"].showgrid then
	...
	if TukuiDB.getscreenresolution == "800x600"
		...
		if TukuiCF["General"].multisampleprotect then
