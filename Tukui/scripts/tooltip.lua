local Tooltip = TukuiCF["Tooltip"]
local Actionbars = TukuiCF["Actionbars"]

if not Tooltip.enable then return end
...
GameTooltip:HookScript("OnUpdate",function(self, ...)
	if self:GetAnchorType() == "ANCHOR_CURSOR" and NeedBackdropBorderRefresh == true and not Tooltip.cursor then
		...
		self:SetBackdropColor(unpack(TukuiCF["Colors"].backdropcolor))
		self:SetBackdropBorderColor(unpack(TukuiCF["Colors"].bordercolor))
	elseif self:GetAnchorType() == "ANCHOR_NONE" then
		if InCombatLockdown() and Tooltip.hidecombat == true then
			self:SetAlpha(0)
		else
			self:SetAlpha(1)
			self:ClearAllPoints()
			if TukuiCF["Bags"].enable and StuffingFrameBags:IsShown() then
				self:ClearAllPoints()
				self:SetPoint("BOTTOMRIGHT", StuffingFrameBags, "TOPRIGHT", 0, TukuiDB.Scale(3))
			elseif PetActionBarFrame:IsShown() then
				if Actionbars.verticalrightbars then
					self:SetPoint("BOTTOMRIGHT", PetActionButton10, "BOTTOMLEFT", TukuiDB.Scale(-3), 0)
				else
					self:SetPoint("BOTTOMRIGHT", PetActionButton10, "TOPRIGHT", 0, TukuiDB.Scale(3))
				end
			elseif UnitHasVehicleUI("player") then
				...
				if Actionbars.verticalrightbars then
					if Actionbars.rightbars == 0 then
						self:SetPoint("BOTTOMRIGHT", TukuiChatRightTabs, "BOTTOMLEFT", -TukuiDB.Scale(3), 0)
					elseif Actionbars.rightbars == 1 then
						self:SetPoint("BOTTOMRIGHT", MultiBarRightButton12, "BOTTOMLEFT", -TukuiDB.Scale(3), 0)
					elseif Actionbars.splitbar or Actionbars.rightbars == 2 then	
						self:SetPoint("BOTTOMRIGHT", MultiBarBottomRightButton12, "BOTTOMLEFT", -TukuiDB.Scale(3), 0)
					elseif (not Actionbars.splitbar) and Actionbars.rightbars == 3 then	
						self:SetPoint("BOTTOMRIGHT", MultiBarLeftButton12, "BOTTOMLEFT", -TukuiDB.Scale(3), 0)
					end
				elseif not Actionbars.verticalrightbars then
					if Actionbars.rightbars == 0 then
						self:SetPoint("BOTTOMRIGHT", TukuiChatRightTabs, "TOPRIGHT", 0, TukuiDB.Scale(3))
					elseif Actionbars.rightbars == 1 then
						self:SetPoint("BOTTOMRIGHT", MultiBarRightButton12, "TOPRIGHT", 0, TukuiDB.Scale(3))
					elseif Actionbars.splitbar or Actionbars.rightbars == 2 then	
						self:SetPoint("BOTTOMRIGHT", MultiBarBottomRightButton12, "TOPRIGHT", 0, TukuiDB.Scale(3))
					elseif (not Actionbars.splitbar) and Actionbars.rightbars == 3 then	
						self:SetPoint("BOTTOMRIGHT", MultiBarLeftButton12, "TOPRIGHT", 0, TukuiDB.Scale(3))
					end
				else
					self:SetPoint("BOTTOMRIGHT", TukuiChatRightTabs, "TOPRIGHT", 0, TukuiDB.Scale(3))
				end
...
GameTooltipStatusBar:SetScript("OnValueChanged", function(self, value)
	...
	if not self.text then
		...
		self.text:SetFont(TukuiCF["Fonts"].font, 12, "THINOUTLINE")
		self.text:Show()
		if unit then
...
healthBar:SetStatusBarTexture(TukuiCF["Textures"].empath)
...
GameTooltip:HookScript("OnTooltipSetUnit", function(self)
	...
	if (self:GetOwner() ~= UIParent and Tooltip.hideuf) then self:Hide() return end
...
local BorderColor = function(self)
	...
	elseif reaction then
		...
		if quality and quality >= 2 then
			...
			self:SetBackdropBorderColor(unpack(TukuiCF["Colors"].bordercolor))
			healthBarBG:SetBackdropBorderColor(unpack(TukuiCF["Colors"].bordercolor))
			healthBar:SetStatusBarColor(unpack(TukuiCF["Colors"].bordercolor))
		end
	end	
end
...
TukuiTooltip:SetScript("OnEvent", function(self)
	...
	TukuiDB.SkinPanel(DropDownList1MenuBackdrop)
	TukuiDB.SkinPanel(DropDownList2MenuBackdrop)
	TukuiDB.SkinPanel(FriendsTooltip)
	TukuiDB.SkinPanel(BNToastFrame)
	
	BNToastFrame:HookScript("OnShow", function(self)
		self:ClearAllPoints()
		self:SetPoint("BOTTOMLEFT", TukuiChatLeftTabs, "TOPLEFT", 0, TukuiDB.Scale(3))
	end)
