...
if not TukuiCF["Bags"].enable then return end

local bags_BACKPACK = {0, 1, 2, 3, 4}
local bags_BANK = {-1, 5, 6, 7, 8, 9, 10, 11}
local BAGSFONT = TukuiCF["Fonts"].font
local ST_NORMAL = 1
local ST_SOULBAG = 2
local ST_SPECIAL = 3
local ST_QUIVER = 4
...
function Stuffing:SlotUpdate(b)
	...
	if not b.frame.lock then
		b.frame:SetBackdropBorderColor(unpack(TukuiCF["Colors"].bordercolor))
	end
...
function Stuffing:CreateBagFrame(w)
	...
	local function bagUpdate(f, ...)
		if w == "Bank" then
			f:SetPoint("BOTTOMLEFT", TukuiChatLeftTabs, "TOPLEFT", 0, TukuiDB.Scale(3))
		else
			if PetActionBarFrame:IsShown() then
				if TukuiCF["Actionbars"].verticalrightbars then
					f:SetPoint("BOTTOMRIGHT", TukuiChatRightTabs, "TOPRIGHT", 0, TukuiDB.Scale(3))
				else
					f:SetPoint("BOTTOMRIGHT", PetActionButton10, "TOPRIGHT", 0, TukuiDB.Scale(3))
				end
			elseif UnitHasVehicleUI("player") then
				...
				if TukuiCF["Actionbars"].verticalrightbars then
					...
					if TukuiCF["Actionbars"].rightbars == 0 then
						f:SetPoint("BOTTOMRIGHT", TukuiChatRightTabs, "TOPRIGHT", 0, TukuiDB.Scale(3))
					elseif TukuiCF["Actionbars"].rightbars == 1 then
						f:SetPoint("BOTTOMRIGHT", MultiBarRightButton12, "TOPRIGHT", 0, TukuiDB.Scale(3))
					elseif TukuiCF["Actionbars"].splitbar or TukuiCF["Actionbars"].rightbars == 2 then	
						f:SetPoint("BOTTOMRIGHT", MultiBarBottomRightButton12, "TOPRIGHT", 0, TukuiDB.Scale(3))
					elseif (not TukuiCF["Actionbars"].splitbar) and TukuiCF["Actionbars"].rightbars == 3 then	
						f:SetPoint("BOTTOMRIGHT", MultiBarLeftButton12, "TOPRIGHT", 0, TukuiDB.Scale(3))
					end
				end
			end
		end
	...
	f.b_close:SetWidth(TukuiDB.Scale(31))
	f.b_close:SetHeight(TukuiDB.Scale(19))
	f.b_close:SetPoint("TOPRIGHT", TukuiDB.Scale(-12), TukuiDB.Scale(-8))
	...
	f.b_close:SetScript("OnEnter", function(self) self:SetBackdropBorderColor(unpack(TukuiCF["Colors"].hoverbordercolor)) end)
    f.b_close:SetScript("OnLeave", function(self) self:SetBackdropBorderColor(unpack(TukuiCF["Colors"].bordercolor)) end)

	f.b_text = f.b_close:CreateFontString(nil, "OVERLAY")
	f.b_text:SetFont(TukuiCF["Fonts"].font, 12)
...
function Stuffing:InitBags()
	...
	gold:SetPoint("RIGHT", f.b_close, "LEFT", TukuiDB.Scale(-6), 0)
...
function Stuffing:Layout(lb)
	...
	if lb then
		bs = bags_BANK
		if TukuiCF["Panels"].infowidth >= 405 then
			cols = 11
		elseif TukuiCF["Panels"].infowidth >= 370 and TukuiCF["Panels"].infowidth < 405 then
			cols = 10
		elseif TukuiCF["Panels"].infowidth >= 335 and TukuiCF["Panels"].infowidth < 370 then
			cols = 9
		else
			cols = 8
		end
			...
			if not (hide_soulbag ~= true and bagType == ST_SOULBAG) then
				...
				for j = 1, bag_cnt do
					...
					elseif (clink and b.qitem) then
						b.frame:SetBackdropBorderColor(1, .3, .3)
					elseif bagType == ST_QUIVER then
						b.frame:SetBackdropBorderColor(.2, .8, .2, .5)
						b.frame.lock = true
					elseif bagType == ST_SOULBAG then
						b.frame:SetBackdropBorderColor(.5, .5, .7, .5)				
						b.frame.lock = true
					elseif bagType == ST_SPECIAL then
						b.frame:SetBackdropBorderColor(.2, .2, .8, .5)
						b.frame.lock = true
					end
