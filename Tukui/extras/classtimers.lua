if not TukuiCF["Unitframes"].enable or not TukuiCF["Classtimers"].enable then return end
...
local MASTER_FONT, STACKS_FONT = { TukuiCF["Fonts"].font, 12 }, { TukuiCF["Fonts"].font, 11, "THICKOUTLINE" }

local PERMANENT_AURA_VALUE = 1

if TukuiCF["Classtimers"].classcolored then
	...
	PLAYER_BAR_COLOR = CreateColor( 55, 55, 55, 1 )
end
...
do
	...
	CreateFramedTexture = function( parent )
		...
		result:SetTexture( unpack ( TukuiCF["Colors"].backdropcolor ) )
		border:SetTexture( unpack ( TukuiCF["Colors"].bordercolor ) )
		background:SetTexture( unpack ( TukuiCF["Colors"].backdropcolor ) )
...
do
	...
	do
		...
		CreateAuraBar = function( parent )
			...
			if TukuiCF["Classtimers"].icons then
			...
			bar:SetStatusBarTexture( TukuiCF["Textures"].empath )
	...
	CreateAuraBarFrame = function( dataSource, parent )
		...
		if TukuiCF["Classtimers"].icons then
			...
			TukuiDB.CreateShadow(iconFrame)
			iconFrame:SetPoint( "TOPLEFT", -TukuiDB.Scale(24), TukuiDB.Scale(2))
			iconFrame:SetPoint( "BOTTOMRIGHT", TukuiDB.Scale(-176), -TukuiDB.Scale(2))
		end
...
local uOffset = TukuiDB.Scale(2)
local iOffset = TukuiDB.Scale(24)
...
if ( playerClass == "DEATHKNIGHT" and TukuiCF["Unitframes"].runebar ) or ( playerClass == "SHAMAN" and TukuiCF["Unitframes"].totembar ) then
	if TukuiCF["Classtimers"].icons then
